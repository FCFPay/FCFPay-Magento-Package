...
//    {
//      return (bool)$this->getValue('min_order_def');
//    }


    public function getDeff($amount,$fiat_amount)
        {
            $percent = (floatval($this->getValue('percent_order_def')) / 100) * $amount;

            if ($percent > floatval($this->getValue('min_order_def'))){
                $percent =  floatval($this->getValue('min_order_def'));
            }
    ...
    public function getModeVarchar(){
        if ($this->getTestMode()){
            return "sandbox";
        }
        return "merchant";
